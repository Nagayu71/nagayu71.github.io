...
最適化（@sec-optimize）と求根アルゴリズム（@sec-root-finding）を使った手法はどちらも，本来連続値をとる現在の所得水準 $w$ を有限個に離散化して，グリッドポイント上で最適貯蓄を計算するという点は共通している．
...
今考えている政策関数は若年期の所得 $w$ を変数にとる貯蓄関数 $a=g(w)$ である．貯蓄関数をパラメトリックに近似するとは、貯蓄関数を**基底関数 (basis function) ** $\{\Psi_m\}_{m=0}^M$ の線形結合

$$
a = g(w) \approx \hat{g}(w;\bm{\theta}) = \sum_{m=0}^M \theta_m\Psi_m(w)
$$ {#eq-parametric-approx}

で近似するということである^[**チェビシェフ多項式 (Chebyshev polynomials)** が基底関数として頻繁に用いられている．]．ここで $\bm{\theta} \equiv \{\theta_m\}_{m=0}^M$ は未知の係数ベクトルであり，推定する対象である．

この段階では，未知の貯蓄関数はきっとパラメトリックな関数 $\hat{g}(w;\bm{\theta})$ で表されるだろうと思っているに過ぎない．

ここで，真の貯蓄関数はオイラー条件（@eq-2.7）を満たすので，それを近似した関数 $\hat{g}(w;\bm{\theta})$ もできるだけオイラー条件を満たしておいて欲しいと思うのは自然な要請である．

つまり射影法とは，オイラー条件を満たす政策関数を探す問題を，（近似なので完全ではないにせよ）関数 $\hat{g}(w;\bm{\theta})$ ができるだけオイラー条件を満たすような係数ベクトル $\bm{\theta}$ を見つける問題に置き換える手法である．
...
多項式 @eq-2.9 の次元を $M=1$ として１次関数 $\hat{g}(w;\bm{\theta}) = \theta_0 + \theta_1 w$ で政策関数を近似しよう．
